...
export default function CreateEyeModal({
  setOpen,
  project,
}: {
  ...
  const createTeam = async () => {
    ...
    try {
      ...
      // call setup api
      const result = await fetch("/api/eye/create", {
        method: "POST",
        headers: {
      ...
      if (result.error) {
        ...
        // 若成功時，轉址
        window.location.href = `/${project.team.slug}/projects/${project.num}/eyes/${result.eye.num}`;
      }
    } catch (error: any) {
